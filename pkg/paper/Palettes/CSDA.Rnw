...
\textit{The colors should not be unappealing.}
...
of fully saturated colors \citep{Tufte-1990-EIN}
...
a computer screen 
\citep[see][for more details]{color:Poynton:2000}
rather than
...
This is the colorspace we advocate in this article.


\textit{The colors in a statistical graphic should cooperate with each other.}
...
These problems can again be addressed by employing an HCL colorspace. 
In addition to
...
\textit{The colors should work everywhere.}
...
variables. Unlike
\pkg{ColorBrewer.org}, we suggest
...
par(lwd=1, new=TRUE)
pie(rep(1,60), radius = 1, col = rainbow(60), 
    border=NA, labels = NA)
par(lwd=3)
pie(rep(1,6), radius = 1, col = rainbow_hcl(6, c = 50, l = 70), 
    labels = 360 * 0:5/6)
par(lwd=1, new=TRUE)
pie(rep(1,60), radius = 1, col = rainbow_hcl(60, c = 50, l = 70), 
    border=NA, labels = NA)
@
