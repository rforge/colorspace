...
This article will describe an appropriate colorspace for
selecting colors.
...
\begin{compactitem}
\item 
The colors in a statistical graphic
...
that those colours should be related to each other.
...
have an intuitive and rational meaning.  However, 
there still remains the issue of providing basic navigation 
tools so that users can conveniently generate color
palettes.  The main part of this article will describe
algorithms for generating color palettes for common 
applications in statistical graphics.
...
In the ideal situation, colors 
should be selected so that they continue to work in any 
context.  For example, different areas of a plot should still be 
distiguishable when printed in black-and-white, or when the person
viewing the graphic is colour-blind.
This article will also briefly address these issues.
...
see][for more details]{color:Poynton:2000}. While being eminently useful for generating
...
of Hue-Saturation-Value (HSV) colors \citep{color:Smith:1978}---which try to capture the
...
spaces discussed in the following]{color:Wiki+HSV:2007}. Although being easy to use,
...
color picker tools \citep{color:Moretti+Lyons:2002,color:Meier+Spalter+Karelitz:2004}
and the basis for many palettes in statistical software packages. A notable
exception is \pkg{ColorBrewer.org} \citep{color:Harrower+Brewer:2003},
...
data. Following \cite{color:Brewer:1999} and \cite{color:Harrower+Brewer:2003}, we distinguish
...
Faithful geyser eruptions data from][]{color:Azzalini+Bowman:1990}---other typical applications
include objective functions with two arguments \citep[as in][]{color:Gneiting+Sevcikova+Percival:2006}
or (physical) measurements on a 2-dimensional grid \citep[as in][]{color:Yang+Buckley+Dudoit:2002}.
...
by varying hue in an HSV color wheel as done by \citet[Figure~4a]{color:Yang+Buckley+Dudoit:2002}
or \citet[Figures~1--4]{color:Gneiting+Sevcikova+Percival:2006}. The palette in the upper left panel
...
%FIXME% examples for sequential palettes:
%%      color:Gelfand+Banerjee+Sirmans:2007
...
if(generate_png) {
  ...
  par(mfrow = c(3, 1), mar = c(0, 0, 0, 0), oma = c(2, 0.2, 0, 0.2))
...
%FIXME% other bad examples for diverging
%%      color:Harezlak+Coull+Laird:2007
%%      color:Tenenhaus+Giron+Viennet:2007
...
\begin{center}
<<votes, fig=TRUE, height=6, width=14, eval=FALSE>>=
...
\begin{figure}[h!]
\begin{center}
<<haireye, fig=TRUE, height=5.5, width=12, eval=FALSE>>=
...
\cite{color:Ihaka:2003} for more details and references.
...
statistics would be bar plots \citep[see][]{color:Ihaka:2003}, pie charts (see Figure~\ref{fig:seats}) 
...
<<barplot2, eval=FALSE>>
## from Takasaki & Kwamura (2007, CSDA, Figure 2, Columns 1-6)
x <- cbind(c( 2,  6, 91,  1), c(35, 30, 20, 15), c(41,  9,  8, 42),
           c(10, 40, 40, 10), c(21, 39, 30, 10), c(35, 13, 17, 35))
mybarplot <- function(x, col = NULL, ...) {
  barplot(x, space = 1.5, names = 1:6, las = 1, col = col)
  grid(lty = 1, ny = NULL, nx = 0, col = "black")
  barplot(x, space = 1.5, names = 1:6, las = 1, col = col, add = TRUE)
}

par(mfrow = c(1, 3))
mybarplot(x, col = c("#8995FF", "#AA0DA4", "#F9FFB9", "#B3FFEE"))
mybarplot(x, col = hcl(c(260, 310, 60, 150), c = 40, l = 80))
mybarplot(x, col = grey.colors(4))
@
