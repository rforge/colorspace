...
\Abstract{
  ...
  and do not introduce optical illusions or systematic bias.
  Based on the perceptually-based Hue-Chroma-Luminance (HCL) color space  
  suitable color palettes are derived for coding categorical data (qualitative palettes) and numerical
...
\fancyhead[CO]{\textit{\normalsize Achim Zeileis, Kurt Hornik, Paul Murrell}} 
\fancyhead[CE]{\textit{\normalsize Escaping RGBland: Selecting Colors for Statistical Graphics}} 
\fancyfoot[LO,LE]{\small Copyright {\copyright} 2009 Elsevier B.V.}
...
\textit{Computational Statistics \& Data Analysis}, \textbf{53}, 3259--3270.
 Copyright {\copyright} 2009 Elsevier B.V. \doi{10.1016/j.csda.2008.11.033}}
}
...
and many statistical graphics in particular. 
...
\textbf{\textit{The colors should not be unappealing.}}
...
A color model typically supported by software packages involves the specification
of colors as Red-Green-Blue (RGB) triplets. However, this specification
corresponds to color generation on a computer screen \citep[see][]{color:Poynton:2000}
rather than corresponding to human color perception. For humans, it is
...
the hue or the brightness of the color. As a consequence, various perceptually-based
...
software packages involves Hue-Saturation-Value (HSV) triplets \citep{color:Smith:1978},
...
highly saturated colors. A perceptually-based color model that mitigates
these problems involves Hue-Chroma-Luminance (HCL) triplets \citep[see][]{color:Ihaka:2003},
resulting from a transformation of CIELUV color space \citep[see][]{color:Wiki+CIELUV:2007}.
This is the color space we advocate in this article.


\textbf{\textit{The colors in a statistical graphic should cooperate with each other.}}
...
\textbf{\textit{The colors should work everywhere.}}
...
%% in the \proglang{R} language \citep{color:R:2008} using the powerful \proglang{R} graphics
...
Some more illustrations are presented in Section~\ref{sec:illustrations}.
Section~\ref{sec:software} offers some general remarks on the implementation in statistical
software as well as some details on the implementation in the \proglang{R}
system for statistical computing \citep{color:R:2008}.
...
if(generate_png) {
  ...
  png(file = "bkde.png", height = 1560, width = 1640, res = 144)
  par(mfrow = c(2, 2), mar = c(4.5, 3.5, 1, 1))
  plot_bkde2D(dens, col = hsv(h = seq(from = 2/3, to = 0, length = 33), v = 0.9, s = 0.9))
  plot_bkde2D(dens, col = rev(heat_hcl(33, h = c(0, 90), c = 0,         l = c(30, 90), power = c(1/5, 1.3))))
  plot_bkde2D(dens, col = rev(heat.colors(33)))
  plot_bkde2D(dens, col = rev(heat_hcl(33, h = c(0, 90), c = c(80, 30), l = c(30, 90), power = c(1/5, 1.3))))
  dev.off()
}
@
...
<<bkde-pal>>=
if(generate_png) {
  png(file = "bkde-pal.png", height = 320, width = 800, res = 144)
  par(mfrow = c(4, 1), mar = c(0.5, 0, 0, 0), oma = c(2, 1.9, 0, 1.9))
  plot(0, 0, type="n", xlim = c(0, 0.03), ylim = c(0, 1),
    xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab="")
  abline(v = seq(from = 0, to = 0.03, length.out = 99), lwd = 6, col = 
    hsv(h = seq(from = 2/3, to = 0, length = 99), v = 0.9, s = 0.9))
  box()
  plot(0, 0, type="n", xlim = c(0, 0.03), ylim = c(0, 1),
    xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab="")
  abline(v = seq(from = 0, to = 0.03, length.out = 99), lwd = 6, col = 
  ...
  plot(0, 0, type="n", xlim = c(0, 0.03), ylim = c(0, 1),
    xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab="")
  abline(v = seq(from = 0, to = 0.03, length.out = 99), lwd = 6, col = 
    rev(heat_hcl(99, h = c(0, 90), c = c(80, 30), l = c(30, 90), power = c(1/5, 1.3))))
  box()
  plot(0, 0, type="n", xlim = c(0, 0.03), ylim = c(0, 1),
    xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab="")
  abline(v = seq(from = 0, to = 0.03, length.out = 99), lwd = 6, col = 
    rev(heat_hcl(99, h = c(0, 90), c = 0,         l = c(30, 90), power = c(1/5, 1.3))))
  box()
  axis(1, cex.axis = 1.8)
  dev.off()
}
...
are (counterclockwise from top left) an HSV-based rainbow, HSV-based heat colors, HCL-based heat colors
and grayscales.}
\end{center}
\end{figure}
...
The latter works somewhat better than the former, however both palettes exhibit several drawbacks.
...
palette \citep[see also][for similar approaches]{color:Ware:1988,color:Levkowitz+Herman:1992}. 
Thus, when converted to a grayscale or printed out on a grayscale printer
...
if(generate_png) {
  ...
  png(file = "nigeria.png", height = 2100, width = 1680, res = 144)
...
<<nigeria-pal>>=
if(generate_png) {
  png(file = "nigeria-pal.png", height = 240, width = 800, res = 144)
  par(mfrow = c(3, 1), mar = c(0.5, 0, 0, 0), oma = c(2, 1.9, 0, 1.9))
  plot(0, 0, type="n", xlim = c(-0.65, 0.65), ylim = c(0, 1),
    xaxs = "i", yaxs = "i", axes = FALSE, xlab = "", ylab="")
  abline(v = seq(from = -0.65, to = 0.65, length.out = 99), lwd = 6, col = 
...
The color palettes employed are (from top to bottom) an HSV-based rainbow and two
HCL-based diverging palettes. In the right panels red-green contrasts are collapsed to
emulate protanopic vision.}
\end{center}
\end{figure}
...
the north-east. \cite{color:Kneib:2006} uses an HSV-based palette coding the deviations by
...
and red hues for negative and postive deviations respectively, but codes neutral values
...
and negative deviations with the same absolute size receive the same perceptual weight
...
For choosing color palettes, it is helpful to have a basic idea how
human color vision works. It has been hypothesized that it evolved in three
...
\cite{color:Kaiser+Boynton:1996}, 
\cite{color:Knoblauch:2002}, \cite{color:Ihaka:2003}, 
and \cite{color:Lumley:2006} for more details and references.
Furthermore, physiological studies have shown that light is captured by three
distinct receptors (so-called cones) in the retina and hence coded in three dimensions
by the human visual system. The perception of this coding, i.e., the subjective
experience of this light, is less well-understood; however, current psychophysical
theories all describe perception in terms of three dimensions \citep{color:Knoblauch:2002}.
Therefore, colors are typically described as locations in 3-dimensional spaces.

The three dimensions used by humans to describe colors are typically:
\begin{itemize}
...
\end{itemize}
Thus, they do not correspond to the axes described above,
but rather to polar coordinates in the color plane (yellow/blue vs.~green/red) plus a third
light/dark axis. Color models that attempt to capture these perceptual axes
are also called perceptually-based color spaces.

\setkeys{Gin}{width=\textwidth}
\begin{figure}[b!]
\begin{center}
<<wheel>>=
png(file = "wheel.png", height = 500, width = 1400, res = 144)
par(mar = rep(0.8, 4), mfrow = c(1, 2))
par(mfrow = c(1, 2), mar = c(1.1, 1, 1.1, 1), oma = c(0, 0, 0, 0))
par(lwd = 3)
pie(rep(1,6), radius = 1, col = rainbow(6), labels = 360 * 0:5/6, init.angle = -30)
par(lwd = 0.5, new = TRUE)
pie(rep(1, 180), radius = 1, col = rainbow(180), border = NA, labels = NA, init.angle = -1)
par(lwd = 3, new = TRUE)
pie(1, radius = 0.55, labels = NA)
par(lwd = 0.5, new = TRUE)
pie(1, radius = 0.55, border = NA, labels = NA)
par(lwd = 3)
pie(rep(1,6), radius = 1, col = rainbow_hcl(6, c = 50, l = 70), labels = 360 * 0:5/6, init.angle = -30)
par(lwd=0.5, new=TRUE)
pie(rep(1, 180), radius = 1, col = rainbow_hcl(180, c = 50, l = 70), border = NA, labels = NA, init.angle = -1)
par(lwd = 3, new = TRUE)
pie(1, radius = 0.55, labels = NA)
par(lwd = 0.5, new = TRUE)
pie(1, radius = 0.55, border = NA, labels = NA)
...
defined by a triplet $(H, S, V)$ with $H \in [0, 360]$ and $S, V \in [0, 100]$.
...
that more closely map to the perception dimensions, the most prominent of which are
the CIELUV and CIELAB spaces developed by the \cite{color:CIE:2004}. 
These color spaces are still not perfect---such an ideal is never
truly attainable because color perception will always
differ between individuals---but they facilitate color matching for humans
\citep{color:Zhang+Montag:2006} and offer a significant
improvement over HSV, as Figure~\ref{fig:wheel} illustrates.


\cite{color:Ihaka:2003}
...
<<pal-q-construct>>=
if(generate_png) {
  png(file = "pal-q-construct.png", height = 960, width = 960, res = 144)
  par(mar = c(0, 0, 0, 0))
...
<<pal-q-examples, fig=TRUE, height=3, width=6>>=
par(mfrow = c(2, 2), mar = c(1, 0, 3, 2))
pal(rainbow_hcl(4, start =  30, end = 300, c = 50, l = 70), main = "dynamic [30, 300]")
pal(rainbow_hcl(4, start =  60, end = 240, c = 50, l = 70), main = "harmonic [60, 240]")
pal(rainbow_hcl(4, start = 270, end = 150, c = 50, l = 70), main = "cold [270, 150]")
pal(rainbow_hcl(4, start =  90, end = -30, c = 50, l = 70), main = "warm [90, -30]")
@
\caption{\label{fig:pal-q} Qualitative palettes: construction and examples.
Top: Construction in the hue/chroma
plane for $L = 70$, the dashed circle correponds to a radius $C = 50$ with chosen angles
$H = 0, 120, 240$. Bottom: Examples by varying hue in different intervals for
given $C = 50$ and $L = 70$.}
\end{center}
\end{figure}
...
or highlighted mosaic displays (see Section~\ref{sec:illustrations}).
...
Figure~\ref{fig:pal-q} (top panel) depicts how the HCL-based color wheel is constructed.
...
in Figure~\ref{fig:pal-q} (bottom panel) which depicts
...
The simplest solution to this task is to employ light/dark contrasts
\citep{color:Levkowitz+Herman:1992}, i.e., rely
...
<<pal-s-construct>>=
if(generate_png) {
  png(file = "pal-s-construct.png", height = 840, width = 1680, res = 144)
  par(mfrow = c(1, 2))
...
<<pal-s-examples, fig=TRUE, height=4, width=6>>=
par(mfrow = c(5, 1), mar = c(0.25, 0, 0.25, 0))
pal(sequential_hcl(12, h = 260,        c = 0,         l = c(30, 90), power = 2.2))
pal(sequential_hcl(12, h = 260,        c = c(80, 0),  l = c(30, 90), power = 2.2))
pal(      heat_hcl(12, h = c(0, 90),   c = c(80, 30), l = c(30, 90), power = c(1/5, 2)))
pal(rev(  heat_hcl(12, h = c(0, -100), c = c(40, 80), l = c(75, 40), power = 1)))
pal(   terrain_hcl(12, h = c(130, 0),  c = c(65, 0),  l = c(45, 90), power = c(1/2, 1.5)))
@

\caption{\label{fig:pal-s} Sequential palettes: construction and examples.
Top: Construction is shown in the chroma/luminance
plane for two hues $H = 260$ (left) and $H = 120$ (right). Colors are chosen by varying
either only luminance or both luminance and chroma. Bottom: Examples by varying
only luminance (first row), chroma and luminance (second row), and hue,
chroma and luminance (remaining rows).}
\end{center}
\end{figure}
...
This strategy is depicted in the top left panel of Figure~\ref{fig:pal-s} for a blue
hue $H = 260$ with $L_{\max} = 90$
and different combinations of maximal chroma ($C_{\max} = 0$, $80$ and $100$,
respecitvely) and minimal luminance ($L_{\min} = 30$, $30$ and $50$, respectively).
The first two combinations are also shown in the first two rows of the bottom panel in Figure~\ref{fig:pal-s}.
The top right panel of Figure~\ref{fig:pal-s} shows that the exact same strategy
...
Such a strategy is adopted for the palettes shown in the last three rows in
Figure~\ref{fig:pal-s} using different pairs of hues as well as different
...
<<pal-d-construct>>=
if(generate_png) {
  png(file = "pal-d-construct.png", height = 960, width = 1068, res = 144)
...
<<pal-d-examples, fig=TRUE, height=3.3, width=6>>=
par(mfrow = c(4, 1), mar = c(0.25, 0, 0.25, 0))
pal(diverge_hcl(7, h = c(260, 0),   c =  80, l = c(30, 90), power = 1.5))
pal(diverge_hcl(7, h = c(260, 0),   c = 100, l = c(50, 90), power = 1))
pal(diverge_hcl(7, h = c(130, 43),  c = 100, l = c(70, 90), power = 1.5))
pal(diverge_hcl(7, h = c(180, 330), c =  59, l = c(75, 95), power = 1.5))
@
\caption{\label{fig:pal-d} Diverging palettes: construction and examples.
Top: Construction is shown in the chroma/luminance
plane for hues $H = 0$ and $H = 260$ (back to back). Colors are chosen by simultaneously 
varying luminance and chroma. Bottom: Examples with different
...
Figure~\ref{fig:pal-d} (top panel) shows the chroma/luminance plane back to back for the
...
As the top panels of Figure~\ref{fig:pal-s} illustrates, the pair of hues should be chosen carefully
...
used in Figure~\ref{fig:pal-d} (top panel) were chosen because they correspond to similar
...
Figure~\ref{fig:pal-d} (bottom panel) shows various examples of conceivable combinations of
...
This section provides several more demonstrations of how 
HCL-based color palettes can be used to produce statistical
plots that are less ``flashy'' and more perceptually balanced,
so that the viewer can
extract information from the plots more easily and more accurately.

\setkeys{Gin}{width=\textwidth}
\begin{figure}[b!]
\begin{center}
<<seats, fig=TRUE, height=4, width=11>>=
par(mar = c(0.8, 1.6, 0, 0), mfrow = c(1, 2))
...
names(seats)[4] <- "Grüne"
Encoding(names(seats)) <- "latin1"
pie(seats, clockwise = TRUE, col = parties_hsv, radius = 0.85)
pie(seats, clockwise = TRUE, col = parties_hcl, radius = 0.85)
@

<<votes, fig=TRUE, height=5.5, width=13>>=
...
Encoding(dimnames(votes)[[1]]) <- "latin1"
...
mosaic(votes, gp = gpar(fill = parties_hsv),
  ...
  margins = unit(c(2.5, 0.5, 1, 11), "lines"),
...
popViewport(2)
@
\caption{\label{fig:election} German election 2005 with HSV-based (left) and HCL-based (right)
qualitative palette. Top: Pie chart for seats in the parliament.
Bottom: Mosaic display for votes by province.}
\end{center}
\end{figure}
...
in the top row of Figure~\ref{fig:election}, visualizing the distribution of seats in the German parliament
...
complex displays such as the mosaic display in the bottom row of Figure~\ref{fig:election}.
This re-uses the same colors and shows the distribution of votes in the 2005 election
...
if(generate_png) {
  ...
  png(file = "scatter.png", height = 700, width = 1680, res = 144)
  par(mfrow = c(1, 2), oma = c(0, 0.1, 0, 0.1), mar = c(1.2, 3, 1.2, 3.3))
...
popViewport(2)
@
\caption{\label{fig:further} Further examples for HSV-based (left) and HCL-based (right)
palettes. Top: Scatter plot with three clusters and qualitative palette.
Bottom: Extended mosaic display for hair and eye color data with diverging palette.}
\end{center}
\end{figure}
...
example, the top row in Figure~\ref{fig:further} depicts a scatter plot with three clusters similar to 
the one shown in \citet[Figure~3]{color:Celeux+Hurn+Robert:2000}. To indicate cluster
...
As our final example, the bottom row in Figure~\ref{fig:further}
\citep[taken from][Figure~11, left/middle]{color:Friendly:2002}
...
or black hair and blue eyes. Comparing the HSV and HCL colors in the bottom row of Figure~\ref{fig:further},
it is shown again that the HCL colors are better balanced (between the red and blue colors)
and less flashy. Even the cells associated with small residuals (below 2) are somewhat easier
to read when shaded in a light grey rather than white. More details and extensions to residual-based
shadings can be found in \cite{color:Zeileis+Meyer+Hornik:2007}.
...
palette can be conveniently computed. Somewhat more work is required if the software
...
to sRGB. The details of these conversions are somewhat technical and tedious (and hence
...
The \proglang{R} system for statistical computing \citep{color:R:2008} provides
an open-source implementation of HCL (and other color spaces) in the package
\pkg{colorspace}, originally written by Ross Ihaka. The coordinate transformations mentioned
above are contained in \proglang{C}~code within \pkg{colorspace} that are easy to
port to other statistical software systems. Version~1.0-0 of
\pkg{colorspace} \citep{color:Ihaka+Murrell+Hornik+Zeileis:2008} also includes
an implementation of all palettes discussed above. 
\citep[Originally, the code for the palettes was in the \pkg{vcd} package,]%
[but it was recently moved to \pkg{colorspace} to be more easily acessible.]%
{color:Meyer+Zeileis+Hornik:2006} Qualitative palettes are provided by
...
Technical documentation along with a large collection of example palettes is available via
\verb/help("rainbow_hcl", package = "colorspace")/. Furthermore, \proglang{R}~code for reproducing
the example palettes in Figures~\ref{fig:pal-q}, \ref{fig:pal-s} and \ref{fig:pal-d}
(and some illustrations) can be accessed via \code{vignette("hcl-colors", package = "colorspace")}.

The default color palettes in the \pkg{ggplot2} package 
\citep{color:Wickham:2008} are also based on HCL colors,
using similar ideas to those discussed in this article.
...
We are thankful to Michael H\"ohle, Ross Ihaka, Thomas Kneib, Ken Knoblauch,
Thomas Lumley, David Meyer, and Brian~D.~Ripley for feedback, suggestions, and discussions.
Furthermore, we are indebted to an anonymous associate editor and three referees
for their reviews that helped improving the manuscript.
