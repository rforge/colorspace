...
if(generate_png) {
  ...
  png(file = "bkde.png", height = 800, width = 830)
...
<<bkde-pal>>=
if(generate_png) {
  png(file = "bkde-pal.png", height = 160, width = 400)
  par(mfrow = c(4, 1), mar = c(0.5, 0, 0, 0), oma = c(2, 1.7, 0, 1.9))
...
\begin{figure}[b!]
\begin{center}
<<wheel, fig=TRUE, height=3.6, width=10>>=
...
pie(rep(1, 90), radius = 1, col = rainbow(90), border = NA, labels = NA, init.angle = -2)
par(lwd = 3, new = TRUE)
...
par(lwd=0.5, new=TRUE)
pie(rep(1, 90), radius = 1, col = rainbow_hcl(90, c = 50, l = 70), border = NA, labels = NA, init.angle = -2)
par(lwd = 3, new = TRUE)
pie(1, radius = 0.55, labels = NA)
par(lwd = 0.5, new = TRUE)
pie(1, radius = 0.55, border = NA, labels = NA)
...
As our final example, Figure~\ref{fig:haireye} \citep[taken from][Figure~11, left/middle]{color:Friendly:2002}
