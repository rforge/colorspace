...
graphics in particular. Statistical software packages allow practicioners
to easily create color graphics which are therefore virtually omnipresent in electronic
...
color choice in such displays is sub-optimal because there is relatively
little guidance about how to choose appropriate colors for a particular visualization
task---and doing so is not a trivial task. There are three main obstacles to overcome:

\paragraph{The colors should not be unappealing.}
It is not necessary for the colors in a statistical plot to reflect the latest
in fashion trends, but basic principles such as avoiding fully saturated colors
should be adhered to. The requirement is not that the user should have
a degree in graphic design, but that the software should provide users with an
intuitive way to select colors and control their basic properties such as how
bright the colors are. Thus, it is necessary to employ a \emph{color model}
or \emph{color space} that describes colors in terms of their perceptual
properties hue, brightness, and colorfulness.

A color model typically supported by software packages is the specification
of colors as Red-Green-Blue (RGB) triplets. However, this specification
does not correspond to human color perception but to color generation on
a computer screen where different color sensations are created by
by mixing different amounts of the primary colors red, green and blue
\citep[see][for more details]{color:Poynton:2000}. For humans, it is
virtually impossible to control the perceptual properties of a color in this
color space because there is no single dimension that corresponds to, for example,
the hue or the brightness of the color. Therefore, various perceptually based
...
software packages are Hue-Saturation-Value (HSV) triplets \citep{color:Smith:1978},
...
properties---furthermore, the use of HSV colors encourages the use of flashy and
highly saturated colors which are good for drawing attention to a plot but hard
to look at for a longer time. A perceptually based color model that avoids
these problems are Hue-Chroma-Luminance (HCL) triplets \citep[see][]{color:Ihaka:2003},
a transformation of CIELUV color space \citep[see]{color:Wiki+CIELUV:2007}.
Albeit being less commonly available in software packages, it is extremely
useful for sepecifying colors intuitively.
...
A natural solution to this task is to vary (at least) one perceptual property
...
however, most commonly based on the HSV model.
...
the others fixed. For example, a change in hue results in a color that may also be
perceptually brighter and more saturated. This means that it is relatively
...
of shaded areas and color-caused optical illusions \citep{color:Cleveland+McGill:1983}.
These problems can again be addressed by employing HCL colors as a more
appropriate perceptually based color model. Above, we argued that HCL space
is useful because it allows humans to understand where within the space
a particular HCL triplet is located. Here, we extend this argument: HCL also allows
us to understand where we are going, i.e., motion within the color space and
...
An approach exemplary in this respect is \pkg{ColorBrewer.org}
\citep{color:Harrower+Brewer:2003}, an online tool for selecting color
schemes for maps. It provides a rich collection of prefabricated palettes
(with a fixed maximal number of colors), supplies information about the
suitability for color-blind viewers or grayscale printing, and guides its
users how to choose a suitable palette for coding various types of
...
information and the latter two are aimed at numerical variables (or ordinal
variables if some numerical coding for its levels is used). Unlike
\pkg{ColorBrewer.org}, we not only provide fixed sets of colors but suggest
