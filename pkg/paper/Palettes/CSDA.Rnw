...
pacakge and color images are therefore virtually omnipresent in electronic
...
task---and doing so is not a trivial task. 
...
a computer screen where different color sensations are created
...
Although this is less commonly available in software packages, it is extremely
...
however, most are commonly based on the HSV model.
