...
graphics in particular.  Furthermore, 
an increasing amount of research is being published and viewed online, making
it easier to include color in published plots.
However, selecting which colors to use for a statistical graphic is
not a trivial task. There are three main obstacles to overcome:

\paragraph{The colors should not be unappealing.}
It is not necessary for the 
colors in a statistical plot to reflect the latest in fashion trends, 
but basic 
principles such as avoiding fully saturated colors should be
adhered to.  The requirement is not that the user should have
a degree in graphic design, but
that the software should allow the user to select colors and provide
control over basic properties such as how bright the colors are.
In order to be able to control basic perceptual features of colors,
it is necessary to work in a \emph{colorspace} that describes 
colors in terms of their perceptual properties.

Typically, software allows the user to specify colors as 
...
see][for more details]{color:Poynton:2000}.
Unfortunately, it is impossible to control
the perceptual properties of a color in this colorspace because there
is no single dimension that corresponds to, for example, the brightness
of the colour.  In
this article we will describe a more appropriate colorspace for
selecting colors.

\paragraph{The colors in a statistical graphic should cooperate with each other.}
The typical purpose 
of color in a statistical graphic is to distinguish between different 
areas or symbols in the plot---to distinguish between different
groups or between different levels of a variable.  This means
that there will typically be several colors, or a \emph{palette}
of colors, used within a plot and 
that those colors should be related to each other.

Part of the solution to this problem is to work in an
appropriate colorspace, as in the previous point, so that 
motion within the colorspace and distances between colors 
have an intuitive and rational meaning.  
For example, another common software 
approach \citep{color:Moretti+Lyons:2002,color:Meier+Spalter+Karelitz:2004} 
is to allow
colors to be specified as Hue-Saturation-Value (HSV) 
triplets \citep{color:Smith:1978},
where each dimension controls a separate perceptual 
property of the color.  
HSV space is a simple transformation of RGB
space 
\citep[see][for more details on HSV colors and links to further information about the other color
spaces discussed in the following]{color:Wiki+HSV:2007} and,
unfortunately, the dimensions in 
HSV space are not truly independent.  
For example,
a change in hue results in a color that may also be
perceptually brighter and more saturated.  
This means that it is relatively difficult to select sets of
HSV coordinates that yield colors that are ``in harmony'' \citep[see][]{color:Munsell:1905}.
For statistical graphics, this is important because it can introduce size distortions in
the perception of shaded areas and color-caused optical illusions \citep{color:Cleveland+McGill:1983}.
...
A different approach to obtaining a color palette is provided by the 
\pkg{ColorBrewer.org} \citep{color:Harrower+Brewer:2003}
online tool for selecting color schemes for maps. It provides a rich
...
various types of information.  The drawback to this tool is that
it only provides a fixed set of colors for each palette and there
is no way to extend the existing palettes.

Following \cite{color:Brewer:1999} and \cite{color:Harrower+Brewer:2003}, we distinguish
...
Unlike \pkg{ColorBrewer.org}, we not only provide fixed sets of colors but suggest
...
taken and how many colors should be selected. The color space we will work in
 is the
Hue-Chroma-Luminance (HCL) color space \citep[see][]{color:Ihaka:2003} and we show how
the general principles suggested can be turned into formulas describing paths of
coordinates in this space.

\paragraph{The colors should work everywhere.}
...
distiguishable when the person
viewing the graphic is color-blind
or when the graphic is printed in black-and-white instead of being
viewed on screen.
These goals cannot always be attained, but we will discuss 
situations where attention should be paid to these issues and
suggest strategies for resolving any problems.
%FIXME% Also mention that the colors should work on different devices.


%FIXME% A sentence or so is needed as a "bridge".
...
\textit{Computational Statistics \& Data Analysis} (Volume~51).
...
by varying hue in an HSV color wheel as done by 
...
increase from a yellow to a red hue while being balanced towards the same gray as in the
grayscale palette. Thus, when converted to a grayscale or printed out on a grayscale printer
(as in the printed version of \textit{Computational Statistics \& Data Analysis}),
...
By balancing the lower right panel towards light/dark contrasts, it is assured that the graphic
is intelligible for color-blind readers and that the same graphic works in
...
of hue $H$ and chroma $C$ are balanced towards the same gray (and thus look virtually identical
...
%% FIXME:  the figures referred to below no longer exist!!
