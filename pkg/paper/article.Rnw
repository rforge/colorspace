...
CIE LUV colorspace, which builds upon a couple of other
earlier transformations of the CIE XYZ colorspace.
...
viewing conditions under which the colour is to be shown.
\footnote{It is relatively straightforward 
...
The white point is expressed as CIE XYZ tristimulus values.
...
high-definition television), which specifies CIE XYZ values
