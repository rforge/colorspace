...
approach is \pkg{weanderson} [@color:wesanderson]. The \pkg{paletteer}
...
(see Figure\ \ref{fig:iris-diamonds}). First, semi-transparent
...
divergingx_palettes(n = 7, plot = TRUE, nrow = 9)
```
...
In Figure\ \ref{fig:brewer-carto-viridis} `specplot()` panels (see also
Section\ \ref{sec:specplot}) are shown for two blue/green/yellow palettes (`RColorBrewer::brewer.pal(7, "YlGnBu")`
...
```{r brewer-carto-viridis, echo = FALSE, fig.width = 6, fig.height = 6, out.width = "0.49\\textwidth", fig.cap = "HCL spectrum of four palettes taken from \\pkg{ColorBrewer.org}, \\pkg{CARTO}, and \\pkg{viridis} (top swatch, solid lines) along with their HCL-based approximations (bottom swatch, dashed lines)."}
specplot(
...
and might need further tweaking in future versions. Thus,
...
in a given situation. Further useful is provided in many sources, including
