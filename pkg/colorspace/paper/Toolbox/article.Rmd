...
abstract: >
  ...
  Using the HCL color model, general strategies for three types of palettes are implemented:
  ...
  To aid selection and application of these palettes, the package also contains scales
...
preamble: >
  ...
  \shortcites{color:colorspace,color:ggplot2pkg}
  \newcommand{\tex}[1]{#1}
...
Color is an integral element of many statistical graphics and data
visualizations. Therefore, colors should be carefully chosen to support all viewers in
accessing the information displayed [@color:Tufte:1990; @color:Brewer:1999; @color:Ware:2004; @color:Wilkinson:2005; @color:Wilke:2019].
However, until relatively recently many software packages have been using color
palettes derived from simple RGB (red-green-blue)
color combinations such as the RGB "rainbow" (or "jet") color palette
with poor perceptual properties. See @color:Hawkins+McNeall+Stephenson:2014
and @color:Stauffer+Mayr+Dabernig:2015 and the references therein for an overview.

To address these problems, many improved color palettes with better perceptual
properties have been receiving increasing attention in the literature
[@color:Harrower+Brewer:2003; @color:Zeileis+Hornik+Murrell:2009; @color:Smith+VanDerWalt:2015; @color:CARTO; @color:Crameri:2018].
Many systems for statistical and scientific computing provide
infrastructure for such color palettes. For example, for \proglang{R} [@color:R] the
list of useful packages encompasses
\pkg{RColorBrewer} [@color:RColorBrewer],
\pkg{viridis} [@color:viridis],
\pkg{rcartocolor} [@color:rcartocolor],
\pkg{wesanderson} [@color:wesanderson],
and \pkg{scico} [@color:scico] among many others.
Furthermore, packages like \pkg{pals} [@color:pals] and \pkg{paletteer} [@color:paletteer]
collect many of the proposed palettes in combination with a unified interface.
Most of these palettes, however, are pre-existing palettes, stored as a
limited set of colors and interpolated as necessary. And even if specific
algorithms have been used in the initial construction of the palettes, these
are often not reflected in the software implementations.

```{r hcl-properties, echo = FALSE, fig.width = 4, fig.height = 2.4, out.width = "0.5\\textwidth", fig.cap = "Axes of the HCL color space. Top: Hue $H$ changes from 0 (red) via 75 (yellow), etc.\\ to 300 (purple) with fixed $C = 60$ and $L = 65$. Center: Chroma $C$ changes from 0 (gray) to 100 (colorful) with fixed $H = 0$ (red) and $L = 65$. Bottom: Luminance $L$ changes from 95 (light) to 15 (dark) with fixed $H = 260$ (blue) and $C = 25$ (low, close to gray)."}
par(mar = c(0.1, 6.1, 0, 0))
plot(0, 0, type = "n", xlim = c(0, 5), ylim = c(0, 3), axes = FALSE, xlab = "", ylab = "")
hcl_prop <- rbind(
  "Hue\n(Type of color)"    = hcl(0:4 * 75, 60, 65),
  "Chroma\n(Colorfulness)"  = hcl(0, 0:4 * 25, 65),
  "Luminance\n(Brightness)" = hcl(260, 25, 95 - 0:4 * 20))
mtext(rownames(hcl_prop), side = 2, at = 2:0 + 0.65, las = 1, line = 5.2, adj = 0, cex = 1)
rect(rep(0:4, each = 3), rep(2:0, 5) + 0.3, rep(1:5, each = 3), rep(3:1, 5), col = hcl_prop, border = "lightgray")
text(rep(0:4, each = 3) + 0.5, rep(2:0, 5) + 0.2, rbind(0:4 * 75, 0:4 * 25, 95 - 0:4 * 20), cex = 0.7)
```


The \pkg{colorspace} package [@color:colorspace] adopts a somewhat different
approach that gives the user direct access to the construction principles underlying
its palettes. These are based on simple trajectories in the perceptually-based
HCL (Hue--Chroma--Luminance) color space [@color:Wiki+HCL] whose axes match those of the human visual
system very well: Hue (type of color, dominant wavelength),
chroma (colorfulness), luminance (brightness), see Figure\ \ref{fig:hcl-properties}.
Thus, utilizing this color model the \pkg{colorspace} package can derive general and adaptable
strategies for color palettes; manipulate individual colors and color palettes;
and assess and visualize the properties of color palettes (beyond simple color swatches).
Specifically, \pkg{colorspace} provides three types of palettes based on the HCL model:
...
A broad collection of prespecified palettes is shipped in the package. In addition, existing
palettes can be easily tweaked and new or adapted palettes registered. The prespecified
palettes include suitable HCL color choices that closely approximate most palettes
from packages \pkg{RColorBrewer}, \pkg{rcartocolor}, and \pkg{viridis} by using only a
small set of hue, chroma, and luminance parameters.

To aid choice and application of these palettes the package provides
(a) scales for use with \pkg{ggplot2} [@color:ggplot2],
(b) \pkg{shiny} [@color:shiny] and \pkg{tcltk} [@color:R] apps for interactive exploration,
(c) visualizations of palette properties, and
(d) accompanying manipulation utilities (like converting to grayscale by desaturation, lighten/darken, and emulation of color vision deficiencies).

The remainder of the paper is organized as follows:
Section\ \ref{sec:tour} gives a first overview of the package's "look & feel" and the general workflow.
Section\ \ref{sec:color_spaces} summarizes the \proglang{S}4 color space classes and methods in the package.
Section\ \ref{sec:hcl_palettes} introduces the extensible collection of HCL-based palettes along with their construction details.
Section\ \ref{sec:palette_visualization} presents the toolbox for palette visualization and assessment.
Section\ \ref{sec:color_vision_deficiency} discusses the implemented techniques for color vision deficiency emulation
that help assess the suitability of colors for colorblind viewers.
Section\ \ref{sec:hclwizard} briefly highlights the interactive color apps from the package.
Some further color manipulation utilities are highlighted in Section\ \ref{sec:manipulation_utilities}
before Section\ \ref{sec:summary} concludes the paper.
...
some of these are illustrated in more detail later):

```{r hcl-palettes-0, echo = TRUE, eval = FALSE}
library("colorspace")
hcl_palettes(plot = TRUE)
```
```{r hcl-palettes, echo = FALSE, fig.height = 6.75, fig.width = 12, fig.cap = "Brief overview of available predefined palettes in \\pkg{colorspace}."}
library("colorspace")
hcl_palettes(plot = TRUE, line = 6)
```

A suitable vector of colors can be easily computed by specifying the desired number
of colors and the palette name (see Figure\ \ref{fig:hcl-palettes} for possible palette names), e.g.,
...
a palette's hue/chroma/luminance parameters can be modified, thus allowing for easy customization of
...
As another example for a sequential palette, we demonstrate how to create a spine plot (see the right
...
employed: a dark purple that is highlighted against a light gray.
...
```{r eustockmarkets-titanic, echo = FALSE, fig.height = 4, fig.width = 10, fig.cap = "Using \\pkg{colorspace} with base \\proglang{R} graphics. Left: Time series plot of log-prices from \\code{EuStockMarkets} data with \\code{qualitative\\_hcl(4, \"Dark 3\")} palette. Right: Spine plot with survival proportions across passenger classes in the \\code{titanic} data with \\code{sequential\\_hcl(2, \"Purples 3\")} palette."}
...
To provide access to the HCL color palettes from within \pkg{ggplot2} graphics [@color:ggplot2; @color:ggplot2pkg]
suitable discrete, continuous, and binned \pkg{ggplot2} color scales are provided.
...
(see the left panel of Figure\ \ref{fig:iris-diamonds}).
...
thereof, see the right panel of Figure\ \ref{fig:iris-diamonds}). The scale function first
...
```{r allplots-qualitative, echo = FALSE, fig.height = 4.5, fig.width = 14, dev = "png", fig.cap = "Palette visualization and assessment for the \\code{qualitative\\_hcl(4, \"Dark 3\")} palette. Left: Demo bar plot. Center: Hue-chroma plane at fixed $L = 60$ in HCL space. Right: HCL spectrum with linearly changing hue (around color wheel), almost constant chroma, and constant luminance."}
...
"wheel" (from degree 0 to 270). Ideally, chroma would have also been constant to completely balance the colors.
...
```{r allplots-sequential, echo = FALSE, fig.height = 4.5, fig.width = 14, dev = "png", fig.cap = "Palette visualization and assessment for the \\code{sequential\\_hcl(9, \"Purples 3\")} palette. Left: Demo heatmap. Center: Chroma-luminance plane at fixed $H = 270$ in HCL space. Right: HCL spectrum with constant hue, triangular chroma, and increasting luminance."}
s9 <- sequential_hcl(9, "Purples 3")
allplots(s9, "heatmap")
```

In Figure\ \ref{fig:allplots-sequential}, a heatmap (based on the well-known Maunga Whau volcano data) is used as a typical
...
```{r color-spaces, fig.width = 9.5, fig.height = 4.5, echo = FALSE, out.width = "0.9\\textwidth", fig.cap = "Relationships among three-dimensional color spaces implemented in \\pkg{colorspace}. Color models that are (or try to be) perceptually-based are displayed with circles, other color models with rectangles."}
par(mar = rep(0, 4))
...
color spaces [@color:Wiki+Colorspace], as the name of the package conveys. Thus, the package helps to map various three-dimensional
...
specifications in many systems based on the corresponding hexadecimal (or simply hex) codes
[@color:Wiki+Webcolors], e.g., in HTML but also in \proglang{R}.
For completeness further standard color models are included as well in the package.
Their connections are illustrated in Figure\ \ref{fig:color-spaces}.
...
The color spaces, implemented in \pkg{colorspace}, along with their corresponding \proglang{S}4 classes
...
* `RGB()` for the classic Red-Green-Blue color model, which mixes three primary colors
  ...
  in many systems. For example, hex color codes are employed in HTML but also in \proglang{R}.
...
All \proglang{S}4 classes for color spaces inherit from a virtual class `color` which is
...
[see the discussion in @color:convertColor]. For more complex conversions, including
different chromatic adaptation algorithms, a more comprehensive color science approach
is implemented in the \proglang{R} package \pkg{colorscience} [@color:colorscience].
Finally, base \proglang{R} also provides
...
for more details and references. Thus, colors can be described using a 3-dimensional space as shown in the left panel
of Figure\ \ref{fig:hcl-axes}.
However, for describing colors in such a space, it is more natural for humans to employ
polar coordinates in the color plane (yellow/blue vs.\ green/red, visualized by the dashed circle in Figure\ \ref{fig:hcl-axes})
...
in the right panel of Figure\ \ref{fig:hcl-axes} [by @color:Horvath+Lipka:2016]. An animated version of the same plot
is provided online by @color:Horvath+Lipka:2017.

```{r human-axes, echo = FALSE, fig.width = 4.5, fig.height = 4.5, fig.show = "hide"}
par(mar = rep(0, 4))
...
text(ax$x, ax$y, c("red", "green", "yellow", "blue", "light", "dark"), col = midgray, pos = c(4, 2, 4, 2, 3, 1))
```

\begin{figure}[t!]
\centering 
\includegraphics[width=0.42\textwidth]{Figures/human-axes-1.pdf} 
\includegraphics[width=0.49\textwidth]{Figures/hcl-wikimedia.png}
\caption{Visualization of axes capturing human color vision (left) and the corresponding HCL color model (right).}\label{fig:hcl-axes}
\end{figure}

The shape of the HCL space is a distorted double cone which is seen best by looking at
vertical slices, i.e., chroma-luminance planes for given hues. For example, the left panel
in Figure\ \ref{fig:hcl-projections} depicts
...
have somewhat irregular shapes. The right panel in Figure\ \ref{fig:hcl-projections} shows such a
hue-chroma plane for moderately light colors (luminance = 70).
At that luminance, green and orange can become much more colorful compared to blue or red.

```{r hcl-projections, echo = FALSE, fig.width = 9.5, fig.height = 4.5, dev = "png", fig.cap = "Vertical (left) and horizontal (right) slices of the HCL space yielding a chroma-luminance plane for given hue and a hue-chroma plane for given luminance, respectively."}
par(mfrow = c(1, 2))
...
Several utilities are available for working with the \proglang{S}4 classes implementing the
...
* `whitepoint()`: Query and change the so-called white point employed in conversions
  ...
  approximate daylight [@color:Poynton].
...
`qualitative_hcl()`, `sequential_hcl()`, and `diverging_hcl()` can be used which are described in more detail
...
(as illustrated in Figure\ \ref{fig:allplots-qualitative}).
...
(e.g., in bar plots, pie charts, or regions in maps), lighter colors (with moderate
chroma and high luminance) such as `"Pastel 1"` or `"Set 3"` are typically less
distracting. By contrast, when coloring points or lines, more flashy
...
For distinguishing colors in the center of the palette, two strategies can be employed:
...
All except the last are inspired by the \pkg{ColorBrewer.org} palettes with
...
```{r hcl-palettes-sequentialm-0, echo = TRUE, eval = FALSE}
hcl_palettes("sequential (multi-hue)", n = 7, plot = TRUE)
```
```{r hcl-palettes-sequentialm, echo = FALSE, fig.width = 8.5, fig.height = 9.0, fig.cap = "Prespecified sequential multi-hue HCL palettes available in \\code{sequential\\_hcl()} in \\pkg{colorspace}."}
hcl_palettes("sequential (multi-hue)", n = 7, plot = TRUE, line = 6)
```
...
As emphasized in Figure\ \ref{fig:hcl-palettes-principles}, luminance is probably the most important
...
different types of palettes (constant vs.\ linear). However, chroma trajectories are probably the most complicated
...
the left and zero intensity $i = 0$ is on the right of each panel. The concrete parameters are:

* Constant: `c1 = 80`.
* Linear: `c1 = 80`, `c2 = 10`, `p1 = 1` (black) vs.\ `p1 = 1.6` (gray).
* Triangular: `c1 = 60`, `cmax = 80`, `c2 = 10`, `p1 = 1` (black) vs.\ `p1 = 1.6` (gray).
...
* \pkg{ColorBrewer.org} [@color:Harrower+Brewer:2003] as provided by the \proglang{R} package \pkg{RColorBrewer} [@color:RColorBrewer].
  See `demo("brewer", package = "colorspace")`.
* \pkg{CARTO} colors [@color:CARTO] as provided by the \proglang{R} package \pkg{rcartocolor} [@color:rcartocolor].
  See `demo("carto", package = "colorspace")`.
* The viridis palettes of @color:Smith+VanDerWalt:2015 developed for \pkg{matplotlib},
  as provided by the \proglang{R} package \pkg{viridis} [@color:viridis].
  See `demo("viridis", package = "colorspace")`.
* The scientific color maps of @color:Crameri:2018 as provided by the \proglang{R} package \pkg{scico} [@color:scico].
...
This uses a slight power transformation with `p1 = 1.1` in the blue arm of the palette but otherwise essentially
...
Meanwhile, base \proglang{R} has also adopted the HCL-based palettes from \pkg{colorspace} into the `hcl.colors()`
function in \pkg{grDevices} [@color:grDevices]. This provides all the named palettes introduced in
\pkg{colorspace} (with the same names, and defaulting to `"Viridis"`) but without the flexibility
to modify or adapt existing palettes.
...
Optionally, swatches emulating color vision deficiencies (see
Section\ \ref{sec:color_vision_deficiency}) can be added by setting `cvd = TRUE`.

Next, we demonstrate a more complex example of a `swatchplot()` with three matrices
of sequential color palettes of blues, purples, reds, and greens
(see Figure\ \ref{fig:swatch-brpg}).
...
For all palettes, luminance increases monotonically to yield a proper sequential palette. However,
the hue and chroma handling is somewhat different to emphasize different parts of the
palette.

* _Single-hue:_ In each palette the hue is fixed and chroma decreases monotonically
...
* _Single-hue (advanced):_ The hue is fixed (as above) but the chroma trajectory is
...
* _Multi-hue (advanced):_ As in the advanced single-hue palette the chroma trajectory
...
By default, the resulting trajectories
in the HCL spectrum are visualized by a simple line plot where the
x-axis gives the ordering of the colors in the palette. The
y-axis depicts the following information:

* Hue is drawn in red and coordinates are indicated on the axis on the right
  with range $[0, 360]$ or (if necessary) $[-360, 360]$.
...
```{r hcl-qualitative, fig.width = 4, fig.height = 4, dev = "png", out.width = "0.5\\textwidth", fig.cap = "Hue-chroma plane with luminance fixed at $L = 70$ along with the qualitative \\texttt{\"Dynamic\"} palette with varying hue $H$ and chroma fixed at $C = 50$."}
...
```{r hcl-sequential, fig.width = 11.5, fig.height = 4.0, dev = "png", fig.cap = "Luminance-chroma planes with variations of blue sequential single-hue palettes (similar to \\texttt{\"Blues 2\"} and \\texttt{\"Blues 3\"}). Left: Linear chroma for $H = 260$. Center: Triangular chroma for $H = 245$. Right: Power-transformed triangular chroma for $H = 245$."}
par(mfrow = c(1, 3))
...
high chroma while this is not possible to the same extent for $H = 245$ because of
the distorted shape of the HCL space. Hence,
...
```{r hcl-multi, fig.width = 11.5, fig.height = 4.0, dev = "png", fig.cap = "Luminance-chroma planes with blue multi-hue palette and triangular chroma (left), blue-yellow multi-hue palette and linear chroma (center), and diverging blue-red palette with balanced linear chroma."}
par(mfrow = c(1, 3))
...
Hence, in the following Figures\ \ref{fig:demoplot-qualitative}--\ref{fig:demoplot-diverging}
some further illustrations are organized by type of
...
non-functional, @color:Machado+Oliveira+Fernandes:2009 provide a unified model of
...
Another example with a diverging palette is available at
...
<https://colorspace.R-Forge.R-project.org/articles/endrainbow.html> for further
illustrations.
...
The poor results for the RGB rainbow palette are contrasted in Figure\ \ref{fig:heatmap-sequential} with a proper sequential
...
`demoplot(x, "heatmap")` where `x` is the color vector to be used, e.g.,
...
misleading impressions of the terrain around Maunga Whau.
...
\pkg{shiny} apps [@color:shiny]. All of these GUIs/apps can be run locally
from within \proglang{R} and are also provided online at <https://hclwizard.org/>.
...
In addition to the \pkg{shiny} version, the _palette constructor_ app is also available
...
The \pkg{shiny} version of the palette constructor GUI is shown in Figure\ \ref{fig:choose_palette}.
It interfaces the `qualitative_hcl()`, `sequential_hcl()`, and `diverging_hcl()`
palettes from Section\ \ref{sec:hcl_palettes}. The GUIs
...
minimal/maximal luminance, and power transformations that control how
...
The app has been influenced considerably by \pkg{ColorBrewer.org} [@color:Harrower+Brewer:2003].
Similarities include the selection of a qualitative, sequential, or diverging palette
from a list of predefined colors along with an example visualization. However, unlike
\pkg{ColorBrewer.org} our \pkg{shiny} app allows tweaking the HCL parameters underlying
each palette. This makes the app much more flexible but also more complex, potentially requiring more
thought and experience. Due to the flexibility, our app cannot automatically judge safety regarding
color vision deficiencies and printers/photocopiers (as \pkg{ColorBrewer.org} does) but instead it allows emulation of color vision
deficiencies and desaturation. Finally, \pkg{ColorBrewer.org} is geared towards cartography
(albeit its palettes are useful much more generally) while our \pkg{shiny} app includes a
broader range of illustrative displays.
...
It supports uploading a raster image in
...
* `adjust_transparency()`: Add, remove, or modify alpha transparency of colors.
...
somewhat too colorful. Conversely, when darkening rather light colors with little chroma, this can result
...
Regardless of the chosen color space, the adjustment of the `L` component by a certain `amount` can occur by two methods, relative
(the default) or absolute. For example `L - 100 * amount` is used for absolute darkening, or
...
overlaying different elements in graphics [@color:Wiki+Webcolors]. An alpha value
...
hex string in \proglang{R} does not provide an explicit alpha transparency, the
...
This paper provides an overview of the broad capabilities of the
...
the palettes, which can be adjusted to the particular needs of a given
data visulization. The paper also provides various tips and tricks
for choosing an effective palette in a given situation. Further useful
guidance is provided in many sources, including:
@color:Ware:1988, @color:Okabe+Ito:2008, @color:Aigner:2010,
@color:Stauffer+Mayr+Dabernig:2015, @color:Zhang:2015, @color:Rost:2018,
@color:Wilke:2019, and @color:Ciechanowski:2019, among many others.

There are other \proglang{R} packages that can complement the palettes provided by \pkg{colorspace}.
\pkg{Polychrome} [@color:Polychrome; @color:Polychrome2] implements strategies
for qualitative palettes with many "categories". While the qualitative palettes
in Section\ \ref{sec:hcl_palettes} yield only about 6--8 clearly distinguishable
colors due to the fixed chroma and luminance, \pkg{Polychrome} relaxes this
restriction and can thus find a larger number of colors in CIELUV space that
are spaced as far apart as possible. It also has several functions for visualizing
color palettes in different ways. 
The palette collection packages \pkg{pals} [@color:pals] and \pkg{paletteer} [@color:paletteer]
also provide a wide range of prespecified palettes, including some qualitative
schemes with many categories. Note that the palettes are quite diverse, though,
and not all of them are equally suitable for coding qualitative information.
The visualization functions in \pkg{colorspace} from Section\ \ref{sec:palette_visualization}
may be helpful in assessing their properties.
\pkg{roloc} [@color:roloc2; @color:roloc] also provides color conversions,
not between numeric color spaces, but rather from numeric color spaces to English color names.
...
In the paper we focus on the more mature \proglang{R} implementation but
replication materials for most examples are also available for
\proglang{Python}.
...
The authors would like to thank the journal editors, the associate editor,
and two anonymous reviewers for their constructive and helpful feedback
