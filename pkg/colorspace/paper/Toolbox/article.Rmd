...
visualizations. Therefore, colors should be carefully chosen to support the information
displayed [@color:Tufte:1990, @color:Brewer:1999, @color:Ware:2004, @color:Wilkinson:2005, @color:Wilke:2019].
...
properties have been suggested in the literature
[@color:Harrower+Brewer:2003, @color:Zeileis+Hornik+Murrell:2009, @color:Smith+VanDerWalt:2015, @color:Crameri:2018]
and also made available in standard software packages including various
packages for \proglang{R} [@color:R] such as 
...
In packages like \pkg{pals} [@color:pals] and \pkg{paletteer} [@color:paletteer]
many of these palettes are collected and provided with a unified interface.
...
algorithms have been used in the first construction of the palettes, these
are often not reflected in the software implementations.

In contrast, the \pkg{colorspace} package [@color:colorspace] adopts a perceptually-based
approach -- the so-called HCL (hue--chroma--luminance) space -- and
establishes a broad range of color palettes through general and adaptable
trajectories in HCL space. In addition to constructing the palettes the HCL
space is also employed to visualize and assess the properties of a given
palette (beyond simple color swatches). The reason for choosing the HCL space
for all of these tools is that its three axes match those of the human visual system very well:
Hue (= type of color, dominant wavelength),
...
user interface.
The \pkg{pals} package also provides a large number of palettes,
but some are just pre-existing palettes, and where the palettes are new
there is no description of a specific algorithm that was used to generate
them.  
