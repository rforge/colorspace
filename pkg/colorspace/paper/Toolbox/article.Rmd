...
suitable discrete and/or continuous \pkg{ggplot2} color scales are provided.
...
from within \proglang{R} and are also provided online at <http://hclwizard.org/>.
...
corresponds to fully opaque. If a color hex string in \proglang{R} does not provide an explicit
alpha transparency, the color is assumed to be fully opaque.

color:Wiki+Webcolors
...
## modify transparency of a color specified by name (`"black"`), hex string
(`"#000000"`), or integer position in the palette (`1`).
...
adjust transparency: only if necessary (`alpha = NULL`), add (`alpha = TRUE`),
remove (`alpha = FALSE`), or modify (`alpha = 0.8`)
