...
abstract: >
  ...
  General strategies for three types of palettes are provided:
...
properties have been suggested in the literature over the last one or two decades
...
approach, utiliting the perceptually-based HCL (Hue--Chroma--Luminance) color space
[@color:Wiki+HCL]. The three axes of the HCL space match those of the human visual
...
Thus, based on this color model the \pkg{colorspace} package can derive general and adaptable
...
A broad collection of prespecified palettes are shipped in the package, existing
...
To aid choice and application of these palettes there are
...
All \proglang{S}4 classes for color spaces inherit from a virtual class `color` which are
...
though [see the discussion in @color:convertColor]. For more complex conversions, including
