...
other color models with rectangles.
...
To make the package self-contained and exactly backward compatible the \proglang{C}
...
To compute the actual color hex codes (representing sRGB coordinates) based on these parameters, the functions
...
In Figure\ \ref{fig:hcl-palettes-qualitative} the available named palettes are shown. The first five
...
```{r hcl-palettes-qualitative, fig.width = 5.5, fig.height = 5.0, out.width = "0.4\\textwidth", fig.cap = "Prespecified qualitative HCL palettes available in \\code{qualitative\\_hcl()} in \\pkg{colorspace}."}
...
As suggested by @color:Zeileis+Murrell+Hornik:2009 `sequential_hcl()` codes the underlying numeric values by a monotonic sequence
...
```{r hcl-palettes-sequentials, fig.width = 6.5, fig.height = 6.0, out.width = "0.5\\textwidth", fig.cap = "Prespecified sequential single-hue HCL palettes available in \\code{sequential\\_hcl()} in \\pkg{colorspace}."}
...
```{r hcl-palettes-diverging, fig.width = 6.5, fig.height = 7.5, out.width = "0.7\\textwidth", fig.cap = "Prespecified diverging HCL palettes available in \\code{diverging\\_hcl()} in \\pkg{colorspace}."}
...
```{r hcl-trajectories, echo = FALSE, results = "hide", fig.width = 7, fig.height = 2.5, fig.cap = "Foo."}
...
* The registration of a palette is only stored for the current session. When
  \proglang{R} is restarted and/or the `colorspace` package reloaded, only the pre-defined
...
```{r brewer-carto-viridis, results = "hide", echo = FALSE, fig.width = 6, fig.height = 6, out.width = "0.49\\textwidth", fig.cap = "Foo."}
specplot(
...
`swatchplot("Label" = sequential_hcl(5))`.
...
```{r swatch-brpg, fig.width = 11.5, fig.height = 3.5, fig.cap = "Foo."}
...
FIXME: Point to previous plots

Finally, the diverging `"Green-Brown"` palette is depicted in the left panel of Figure\ \ref{fig:specplot}.
...
```{r specplot, fig.width = 5, fig.height = 7, out.width = "0.49\\textwidth", fig.cap = "Foo."}
...
```{r hcl-sequential, fig.width = 11.5, fig.height = 4.0, fig.cap = "Foo.", dev = "png", dpi = 150}
par(mfrow = c(1, 3))
...
```{r hcl-multi, fig.width = 11.5, fig.height = 4.0, fig.cap = "Foo.", dev = "png", dpi = 150}
par(mfrow = c(1, 3))
...
```{r demoplot-all, echo = FALSE, fig.width = 8.5, fig.height = 8.5, fig.cap = "Foo."}
par(mfrow = c(3, 3))
...
```{r demoplot-qualitative, fig.width = 8.5, fig.height = 3.0, fig.cap = "Foo."}
par(mfrow = c(1, 3))
...
```{r demoplot-sequential, fig.width = 8.5, fig.height = 3.0, fig.cap = "Foo."}
par(mfrow = c(1, 3))
...
```{r demoplot-diverging, fig.width = 8.5, fig.height = 3.0, fig.cap = "Foo."}
par(mfrow = c(1, 3))
...
```{r demoplot-dark, fig.width = 8.5, fig.height = 6.0, fig.cap = "Foo."}
...
```{r volcano, echo = FALSE, fig.height = 3, fig.width = 5.5, dev = "png", fig.cap = "Foo.", dpi = 100, out.width = "0.5\\textwidth"}
z <- 2 * volcano
x <- 10 * (1:nrow(z))
y <- 10 * (1:ncol(z))
par(mar = rep(0, 4))
...
```{r heatmap-sequential, echo = FALSE, fig.height = 15, fig.width = 9, out.width = "0.85\\textwidth", fig.pos = "p!", fig.cap = "Foo."}
...
\pkg{shiny} apps [@color:shiny]. All of these GUIs or apps can either be run locally
from within \proglang{R} but are also provided at <http://hclwizard.org/>.
...
In addition to the \pkg{shiny} app, the **palette constructor** app is also available as a Tcl/Tk GUI via \proglang{R} package
...
by default starts the \pkg{shiny} version.
...
```{r choose_color, echo = FALSE, fig.cap="Foo."}
...
\caption[Foo]{Foo.}\label{fig:cvd_emulator}
\end{figure}
...
for use in \proglang{R}.

For illustration, a few simple examples are presented in Figure\ \ref{fig:desaturate-named}. More examples in the
...
First, `desaturate()` is used to desaturate a vector of \proglang{R} color names:
...
```{r desaturate-wheel, out.width = "0.5\\textwidth", fig.cap = "Foo."}
...
```{r lighten-darken, fig.height = 3.5, fig.width = 4.5, out.width = "0.5\\textwidth", fig.cap = "Foo."}
