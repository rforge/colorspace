...
```{r hcl-palettes, fig.height = 6.75, fig.width = 12, fig.cap = "Brief overview of available predefined palettes in \\pkg{colorspace}."}
library("colorspace")
...
```{r allplots-sequential, echo = FALSE, fig.height = 4.5, fig.width = 14, dev = "png", fig.cap = "Palette visualization and assessment for the \\code{sequential\\_hcl(4, \"Purples 3\")} palette. Left: Demo heatmap. Center: Chroma-luminance plane at fixed $H = 270$ in HCL space. Right: HCL spectrum with constant hue, triangular chroma, and increasting luminance."}
s9 <- sequential_hcl(9, "Purples 3")
allplots(s9, "heatmap")
```
...
All except the last one are inspired by the \pkg{ColorBrewer.org} palettes with
...
* When registering a new palette with a previously-used name, the old palette
  gets overwritten. We recommend not to overwrite
...
```{r hcl-sequential, fig.width = 11.5, fig.height = 4.0, dev = "png", fig.cap = "Luminance-chroma plane with variations of blue sequential single-hue palettes (similar to \\texttt{\"Blues 2\"} and \\texttt{\"Blues 3\"}). Left: Linear chroma for $H = 260$. Center: Triangular chroma for $H = 245$. Right: Power-transformed triangular chroma for $H = 245$."}
par(mfrow = c(1, 3))
...
The \pkg{shiny} version of this GUI is shown in Figure\ \ref{fig:choose_palette}.
...
\pkg{ColorBrewer.org} our \pkg{shiny} app allows to tweak the HCL parameters underlying
...
color vision deficiencies and printers/photocopiers (as \pkg{ColorBrewer.org} does) but instead it allows to emulate color vision
