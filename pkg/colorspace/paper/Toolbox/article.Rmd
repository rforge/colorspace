...
* \pkg{Polychrome} [@color:Polychrome, @color:Polychrome2]: qualitative palettes with more "categories". This can generate 
  palettes with the (a large number of) 
  colours spaced far apart from each other in Luv space.
  It also has functions for visualising colour palettes in different ways 
  (https://www.biorxiv.org/content/biorxiv/early/2018/04/18/303883.full.pdf)
* \pkg{roloc} [@color:roloc,@color:roloc2]: colour conversions, but not between
  numeric colour spaces;  instead, from numeric colour spaces to
  colour names.  
  (https://stattech.wordpress.fos.auckland.ac.nz/2018/01/25/2018-01-generating-colour-names-the-roloc-package-for-r/)
* \pkg{colorscience} [@color:colorscience]: This package is for Colour
Science.  It has data sets, does more complex conversions (including
different chromatic adaptation algorithms).  
So more sophisticated than \pkg{colorspace}
in terms of flexibility and low-level detail, BUT it does not do
palette-generation.
