...
\section{Robust Color Palettes for Data Visualization}
...
\begin{itemize}
  ...
  \item Color vision deficiencies affect $\sim 8$\% of male and $\sim 0.5$\% of female viewers.
...
p <- protan(palette.colors(8, "R3")[c(1:3, 5)])
<<EuStockMarkets0>>
<<EuStockMarketsLegend1>>
@
}%
...
p <- protan(palette.colors(8, "R4")[c(1:3, 5)])
<<EuStockMarkets0>>
<<EuStockMarketsLegend2>>
@
}%
...
p <- protan(palette.colors(4, "Okabe-Ito")[c(1, 4, 2, 3)])
<<EuStockMarkets0>>
<<EuStockMarketsLegend2>>
@
}%
...
Hard to distinguish for protanope viewers
...
{\footnotesize
Source: Mara Averick via Twitter
}
...
\textbf{Base R:} Neglected better color palettes for a long time.

\bigskip

\textbf{Earlier packages:} RColorBrewer, colorspace, ggplot2, viridis, rcartocolor, Polychrome, pals, paletteer, \dots

\bigskip

\textbf{Thus:} Many good palettes easily available.
...
\vspace*{-0.5em}
\begin{footnotesize} 
Probability of wind speeds $>$ 39\,mph (63\,km\,h$^{-1}$), 2019-08-30--2019-09-04.
Source: (\href{https://noaa.gov}{noaa.gov})
\end{footnotesize} 
...
{\footnotesize
Source: U.S. president via Twitter (2019-09-05)}
\end{minipage}
...
Movie: \emph{Todo sobre mi madre} (\emph{All About My Mother}, 1999)

\medskip

Picture source: Sony Pictures Classics via MoMA

\medskip

\only<2->{Palette: Hadley Mendelsohn}
}
...
\begin{center}
{\footnotesize
Palette: \only<1>{Todo sobre mi madre}\only<2>{OrRd (ColorBrewer.org, HCL version)}\phantom{Org}}
\end{center} 
...
Movie: \emph{Tacones lejanos} (\emph{High Heels}, 1991)

\medskip

Picture source: El Deseo S.A. via Twitter

\medskip

\only<2->{Palette: Bibiana Fernandez}
}
...
Web: \url{https://colorspace.R-Forge.R-project.org/}

\smallskip

Twitter: \href{https://twitter.com/AchimZeileis}{\tt @AchimZeileis}
