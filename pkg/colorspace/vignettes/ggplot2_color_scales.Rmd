...
ggplot(df_long, aes(var1, var2, fill=cor)) + 
  ...
  scale_fill_continuous_diverging("Purple-Brown")
```
...
And now the same plot with some palette customizations. We adjust the luminance at the end (`l1`) and in the middle (`l2`), as well as the curve describing how quickly the colors change (`p1`).
```{r fig.width = 6.5, fig.asp = 1., fig.align = 'center'}
gg + scale_fill_continuous_diverging(palette = "Blue-Red 3", l1 = 20, l2 = 100, p1 = 1)
```
